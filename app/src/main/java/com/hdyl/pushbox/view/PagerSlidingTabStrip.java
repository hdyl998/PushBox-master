...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int indicatorOffset = 0;// 指示器偏移左右边的距离
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		indicatorOffset = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, indicatorOffset, dm);
		...
		tab.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (tab instanceof TextView) {
					((TextView) tab).setTextColor(position == currentPosition ? indicatorColor : tabTextColor);
				}
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				tab.setTextColor(i == currentPosition ? indicatorTextColor : tabTextColor);
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
			...
			// TextView leftView=(TextView) tabsContainer.getChildAt(position +
			// 1);
		...
		public void onPageSelected(int position) {
			...
			if (indicatorColor != tabTextColor) {
				for (int i = 0; i < tabCount; i++) {
					View v = tabsContainer.getChildAt(i);
					if (v instanceof TextView) {
						TextView tab = (TextView) v;
